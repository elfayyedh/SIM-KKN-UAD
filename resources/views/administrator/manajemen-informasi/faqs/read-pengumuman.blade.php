        ...
        <div class="container-fluid">
            ...
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <div class="card-body">
                            <div class="card-title text-muted fw-bold">Buat/Edit Pengumuman</div>
                            <form action="{{ route('informasi.faq.store') }}" id="faq_form" method="POST">
                                @csrf
                                <div class="form-group mb-3">
                                    <label for="pertanyaan" class="form-label">Judul <span
                                            class="text-danger">*</span></label>
                                    <input type="text" required
                                        class="form-control @error('pertanyaan') is-invalid @enderror" id="pertanyaan"
                                        name="judul" placeholder="Masukkan judul pengumuman">
                                    @error('pertanyaan')
                                ...
                                <div class="form-group mb-3">
                                    <label for="jawaban" class="form-label">Isi <span class="text-danger">*</span></label>
                                    <textarea name="isi" id="ckeditor-classic"></textarea>
                                    @error('isi')
                                        <small class="text-danger">{{ $message }}</small>
                                    @enderror
                                </div>
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-body">
                            <div class="card-title text-muted fw-bold">Daftar Pengumuman</div>
                                        ...
                                        @foreach ($data as $item)
                                            <tr>
                                                ...
                                                <td class="text-wrap" style="max-width: 400px; min-width: 150px;">
                                                    {!! $item->isi !!}</td>
                                                <td>
                                                    <form action="{{ route('informasi.faq.setFaqStatus') }}" method="POST">
                                                        @csrf
                                                        <input type="hidden" name="id" value="{{ $item->id }}">
                                                        <input type="hidden" name="status" value="{{ $item->status }}">
                                                        <button
                                                            class="btn btn-{{ $item->status == true ? 'success' : 'warning' }} btn-sm">
                                                            {{ $item->status == true ? 'Sembunyikan' : 'Tampilkan' }}
                                                        </button>
                                                    </form>
                                                </td>
                                                <td>
                                                    <a href="#tambah_faq" data-pertanyaan="{{ $item->judul }}"
                                                        data-jawaban="{{ $item->isi }}" data-id="{{ $item->id }}"
...
@section('pageScript')
    ...
    <script src="{{ asset('assets/js/init/administrator/read-pengumuman.init.js') }}"></script>
@endsection
